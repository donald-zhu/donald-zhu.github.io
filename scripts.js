...
class Slide {
    ...
    change(increment) {
        ...
        for (const obj in this.adjustCallback) this.adjust(this.adjustCallback[obj])
        pc4Cursor()
    }
    ...
    display(...args) {
        ...
        for (let i = 0; i < itr; i++) {
            ...
            if (typeof elem == 'string') document.querySelector(elem).style.display = style
            else if (typeof elem == 'object') elem.style.display = style
        }
    }
    ...
    page() {
        ...
        for (let i = 0; i < this.pgList.length; i++) {
            ...
            if (i !== this.current &&
                page.style.display !== 'none') this.display(page, 'none')
        }
    }

    text(overflow, captionHeight) {
        document.body.style.overflow = overflow
        if (captionHeight) cp().style.height = chldnH(cp()) + 'px'
    }

    image() {
        const img = cp().querySelector('img'),
    ...
    constructor() {
        ...
        this.mousemove = [
            ['body', e => {
                ...
                if (!hasClass('endpage')) {
                    document.body.style.cursor = `-webkit-image-set(url(cursor/${windowPosition(e) ? 'prev' : 'next'}_clr.svg) 2.5x) 20 20, ${windowPosition(e) ? 'w-resize' : 'e-resize'}`
                } else {
...
class FlipThrough {
    ...
    flip(direction) {
        ...
        if (this.pcNum == 'pc3') {
            this.dom.style.cursor =
                `-webkit-image-set(url(cursor/${(this.clickP - this.left) < this.half ? 'prev' : 'next'
    ...
    cursor() {
        ...
        this.dom.addEventListener('mousemove', e => {
            this.dom.style.cursor =
                `-webkit-image-set(url(cursor/${position(e) ? 'prev' : 'next'}_${this.pcNum == 'pc3' ? (!!this.blue.find(n => n == this.currentPgNum) ?
                    'bl' : 'yt') : 'yt'
...
class Loader {
    ...
    populateOthers() {
        ...
        for (let i = 0; i < 2; i++) {
            const n = ftPc[i + 1]
            for (let ii = 0; ii < pageAmt[i + 1]; ii++)
                this.others.push(`images/pc${n}/fullbook/f${ii + 1}.jpg`)
            this.others.push(`images/pc${n}/fullbook/f1_hover.jpg`)
        }
